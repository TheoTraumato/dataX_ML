...
params = {'criterion': ['gini', 'entropy'], 'max_depth': range(2, 6), 'min_samples_split': range(2, 8, 2),
          'min_samples_leaf': range(2, 8, 2)}
clf = GridSearchCV(tree, params, scoring='neg_log_loss', cv=5)
...
plt.show()
y_pred = tree.predict(x_test)
print("Accuracy for Prep. + Grid model: ", accuracy_score(y_test, y_pred))
print("Precision for Prep. + Grid model: ", precision_score(y_test, y_pred))
print("Recall for Prep. + Grid model: ", recall_score(y_test, y_pred))
print("F1-score for Prep. + Grid model: ", sklearn.metrics.f1_score(y_test, y_pred))
"""Auswertung"""
logreg_matrix = confusion_matrix(y_test, y_pred)
plot_confusion_matrix(logreg_matrix, classes=['churn=1', 'churn=0'], normalize=False,
                      title='Confusion matrix logreg optimized')
...
clf = GridSearchCV(forest, params, scoring='neg_log_loss', cv=5, verbose=2)
clf.fit(x_train, y_train)
...
print("Accuracy for forest model: ", accuracy_score(y_test, y_pred))
print("Precision for forest model: ", precision_score(y_test, y_pred))
print("Recall for forest model: ", recall_score(y_test, y_pred))
print("F1-score for forest model: ", sklearn.metrics.f1_score(y_test, y_pred))
